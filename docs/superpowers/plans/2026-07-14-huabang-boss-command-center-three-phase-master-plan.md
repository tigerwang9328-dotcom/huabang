...
状态：自动化证据契约已通过；已为四个真实触发类型各准备 3 条样本：R001 ID 276/277/278（低销售），R005 ID 219/220/221（高折扣），R006 ID 283/284/285（负库存），R010 ID 286/287/288（高周转天数）；R023 当前仅有真实样本 ID 289（VIP 低折扣），不能伪造另外 2 条。13 个生产详情接口均返回 200、每条均有 1 条原始证据，未登录访问返回 401。经营快照库存读取 2026-07-15 05:30 后快照，R006/R010 则固定留存 2026-07-14 经营日历史库存证据，两者用途不同。老板仍需完成人工业务判断；未触发或不足 3 条的规则不能伪造样本补足。

- [ ] **Step 2: 验证利润门禁**

费用完整时与财务核准表一致；费用不完整时只显示毛利和缺失来源，不显示经营利润结论。

状态：费用完整和不完整两条自动门禁均已通过。2026-07-14 毛利额 6,346.32 元、毛利率 74.48%，成本状态 `ready`；八类费用覆盖率为 0。钉钉财务表仅有 3 条 6 月报销记录，其中 2 条仍在审批中，没有 7 月14日完整核准费用，不能用于计算当日经营利润；仍待财务核准表和老板签字。

- [ ] **Step 3: 验证 VIP 分层与任务闭环**

抽查分层证据，完成一条 VIP 唤醒任务从草稿到关闭的全流程，并验证通知失败不丢任务。

状态：分层、状态机和通知失败隔离已自动验证；真实 VIP 唤醒任务已选定 `/app/task/1715`（会员 000344279445、门店 185805）。该任务候选导购为空，负责人接口仅降级返回 3 个 `mapping_pending` 系统账号；钉钉档案中的“代积芳”属于商品部、“王小虎”属于公司根部门，均无工号、岗位或门店 185805 映射，不能自动认定负责人。主管需先在 `/app/system/users` 绑定真实员工与门店 185805，再完成派发、反馈、复查、关闭并签字。
...
**2026-07-14 验收记录：** 已基于会员分层快照生成每日 VIP 行动候选，推荐商品仅保留会员归属门店当前有外穿衣物库存的款号；可信单一原导购可预填，其他情况必须由主管选择具体责任人并确认话术，系统不自动联系会员。当前生产账号尚未维护员工编号和门店映射，接口会如实标记“门店映射待补”，仅在没有门店绑定人员时降级返回具备任务反馈权限的未绑定账号供主管人工确认，不伪装成原导购。生产任务 1715 的候选导购为空，负责人接口只返回 3 个 `mapping_pending` 账号；钉钉档案中的“代积芳”属于商品部、“王小虎”属于公司根部门，均无工号、岗位或门店 185805 映射，因此仍需主管先完成真实员工门店绑定。跟进复用现有任务状态机，结构化保存联系、到店、成交、百胜小票、未成交原因和下次跟进时间；行动成交额以会员、门店、日期均核验通过的百胜小票金额为准，无法证明发生在确认后的消费仅列自然复购。真实数据库回滚演练识别 550 个候选，首次生成 550 个草稿、同日第二次生成 0 个，幂等成立且未落正式数据。后端全量 `463 passed, 1 skipped`，前端 `126 passed`，类型检查和生产构建通过。
...
**Task 17 验收记录（2026-07-14）：**

- 新增 `/mobile/command-center`，直接返回 Web 同源 `dm_boss_daily_report` 快照；旧 `/mobile/overview` 保留兼容。
- 移动首页使用同一 `core_metrics`，显示经营结论、销售、实收、毛利状态、库存、VIP、重大异常和待处理任务；首次进入读取服务器最新可用日报。
- 经营预警页接入现有 `/task/*` 权限与状态机，可查看证据、选择责任人、确认派发、复查和关闭，不复制任务逻辑。
- 弱网保留最近一次成功快照并统一标记 `stale`；缺失指标显示“待接入”，不回填零。
- 后端全量测试 `465 passed, 1 skipped`；小程序 Node 测试 `5 passed`；微信开发者工具官方预览编译通过，包体 `139.3 KB`。
- 小屏断言覆盖 320/375/390px；390px 微信模拟器实测首页 7 张指标卡无重叠，行动页成功加载 20/588 条真实任务并保持按钮同排。
...
状态：VIP 真实样本已固定为 `/app/task/1715`，但生产反馈和复查仍为 0。2026-07-14 归属七类来源中门店交易、跨店历史、退款记录 3 类为 `ready`；抖音订单级绑定、已核验责任导购、导购到门店排班、改单前后日志 4 类为 `pending_data`，真实归属异常为 0，因此本步保持未完成且不制造验收数据，也不从会员注册门店、考勤部门或最终小票推定责任人。

- [x] **Step 3: 验证 Web 与移动一致性**
...
**Task 18 阶段验收记录（2026-07-15）：**

- AI 固定场景已覆盖缺费用、缺客流、库存过期和模型不可用，模板回退不把缺失值当零，不输出未经财务门禁的确定性盈亏，行动始终要求人工确认。
- 归属/VIP 任务状态机及审计字段已自动验证；2026-07-15 完整周期后生产共有 599 条任务，其中 594 条草稿、5 条待处理，尚无真实反馈、复查或关闭记录，因此真实业务闭环仍待老板签字。
- 2026-07-13 Web 与移动接口逐字段核对一致：销售 13,455 元、实收 8,820 元、库存金额 2,401,938.52 元、VIP 余额 1,118,709.26 元、重大异常 7 项、待处理任务 38 项、退货 0 元。
- 退货金额已接百胜 `baison_pos.refund_amount`：2026-07-06 为 54 元，2026-07-12/13/14 为 0 元，状态均为 `ready`；经营总览在 06:30 前也使用完整小票同步的同口径实时值。
- 后端全量测试 490 passed、前端 131 项测试、类型检查、生产构建、20 组 Playwright 桌面/移动检查、退货桌面/移动补充验收和前端原子回滚机制均已通过；毛利率状态已增加正销售、零销售、成本完整和成本不完整边界回归。数据库为 `222d5e6f7081 (head)`，一步回滚证据沿用第二阶段同一迁移链实测。
- 2026-07-15 完成真实完整周期：04:00 小票/退货成功；05:30 库存首跑发现日期字符串绑定问题，修复后 06:10 按同一锁入口成功，10 编码、82,233 行、原始库存 24,356 件、外穿衣物 18,504 件；06:30 日报首跑发现 R023 公司级空参数类型问题，修复后 06:33 成功，第二次重跑新增任务草稿为 0；06:40 健康检查返回 `healthy=true`、`issues=[]`，未生成 `HEALTH-20260714` 草稿。
- Step 5 已完成；Step 2 的真实业务闭环和 Step 6 老板最终签字仍需人工完成，不能用测试数据替代。
