...
状态：自动化证据契约已通过；2026-07-14 当前业务日已为三个真实触发类型各准备 3 条样本：R001 ID 276/277/278（低销售），R006 ID 283/284/285（负库存），R010 ID 286/287/288（高周转天数），均可通过 `/app/warning?exception_id={id}` 查看来源表、字段值、来源时间和下钻。9 个生产详情接口均返回 200、每条均有 1 条原始证据，未登录访问返回 401。经营快照库存读取 2026-07-15 05:30 后快照，R006/R010 则固定留存 2026-07-14 经营日历史库存证据，两者用途不同。老板仍需完成人工业务判断；未触发的规则不能伪造样本补足。
