...
- 主标签每 5 分钟采集一次视频；已学到的其他业务模板按较低频率重放。不要通过多开标签提高频率。
...
SELECT account_id, status, last_seen_at, last_success_at,
       last_event_id, last_error_at, last_error
FROM app.life_data_collector_state
WHERE account_id = '1798826701211732';
```

`status='online'` 只是最近入站记录，不能单独证明浏览器此刻在线；应以 `last_seen_at`、`last_success_at` 是否持续更新为准。浏览器浮层“队列数”才是本地待重传数量。
