...
- [ ] 由财务负责人确认法人、组织、账套、账簿、会计政策、期间、币种、科目/维度、历史截止日、当前账启用日和报表法定性；本会话已授权忽略形式签字，但未被数据证据确认的事实项仍只允许只读调查，详见 `deployment-decisions.md`。
- [ ] 导出金蝶源快照清单及哈希，并按账套/年度/期间/科目/凭证/分录/余额建立基线；记录缺口。
- [ ] 运行现有财务测试、前端类型检查和构建，仅记录实际输出；任何既有失败先进入风险登记，不以新设计掩盖。

**Go:** 财务口径事实已由可核对数据证据确认（形式签字可按 `deployment-decisions.md` 的用户授权例外处理）、历史与当前覆盖连续或存在已批准且阻断正式报表的 `coverage_gap`、生产发布与恢复事实已记录。
