...
| `fin_history.staging_*`、`import_batch`、`import_checkpoint`、已发布视图/触发器 | Phase 4 | `add finance history publication guards` revision |
| `opening_balance_batch`、`coverage_gap` | Phase 4.5 | `add finance opening balance boundary` revision |
...
- [ ] 创建并核对 `opening_balance_batch_id/status/approved_by/locked_at`；第一张正式凭证过账前仅可废弃并重建期初批次，第一张正式凭证过账后及首次结账后不得直接变更期初，只能在当前开放期间创建调整凭证。
...
- [ ] 在 `api-contract.md` 逐项映射旧 `/api/v1/finance/*` 到 V2 只读代理、V2 草稿适配或 410，并记录弃用日期；旧审核/过账不存在可写映射。
...
- [ ] 演练原子切换：旧制单正常 → V2 只读验收 → V2 凭证工作台/权限验收 → 短时写入冻结 → 旧系统最终增量提取与核对 → 仅试点范围开启 V2 制单 → 关闭/转接旧制单 → 日对账后审核 → 再开启人工过账。演练失败时恢复旧制单或保持冻结，不得形成未计划停摆。
...
**Go:** 此阶段通过前，`draft_enabled`、`review_enabled` 和 `post_enabled` 必须为关闭状态；通过后按原子切换流程仅向批准的试点账簿、组织和角色开放。
