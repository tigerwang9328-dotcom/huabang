...
| 实际执行前置 | 服务器的恢复副本 `huabang_ai_finance_drill_20260729_r2` 已存在且处于 baseline，但当前 SSH 会话的 `sudo -n` 明确被拒绝，不能安全取得或代填管理员凭据。 | 生产主机只读预检，2026-07-30 | 生产控制面 | 恢复副本实际 upgrade/downgrade/触发器/权限演练仍未完成；生产迁移继续 No-Go。 |

**下一 Gate：** 以交互式受控 sudo 或由 PostgreSQL 管理员执行该恢复副本脚本，并保存升级、降级、再升级和 `verify_finance_database_roles.py` 的原始结果。成功前不得部署 `016cfd3c1454`、`c82e5a1f9d70` 或 `e951b2d0a6c4`。
