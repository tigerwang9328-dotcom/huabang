...
## 华邦中台来源候选核验（2026-07-30，生产只读）

| 来源候选 | 已核实事实 | 结论 |
| --- | --- | --- |
| `dwd.dwd_sales_detail` | 466 条、466 个唯一 `detail_no`，仅覆盖 2026-06-14 至 2026-06-16；具备订单、门店、商品、金额和成本字段，但没有收款/结算、税、法人、会计期间或科目映射事实。 | 只能作为未来只读来源快照候选，不能直接生成会计凭证。 |
| `dwd.dwd_return_detail` | 1 条，日期为 2026-06-16；有原订单号、金额和成本，但同样缺少完整结算与会计口径。 | 只能与受控销售来源规则一起做 preview，当前不可记账。 |
| `dwd.dwd_finance_expense`、`dwd.dwd_finance_cash` | 均为 0 条。 | 费用与现金/银行来源未接通，不得以 0 当作已核对余额或完整费用。 |

**来源接入 No-Go：** 禁止将 DWD 经营明细或汇总直接写入 `fin_current`，禁止自动草稿。未来仅通过版本化来源收件箱和 dry-run/preview，并在取得法人/组织/账簿范围、收款结算、税务和映射规则证据后再评审。

## 来源收件箱与 dry-run 框架（2026-07-30，本地）

| 项目 | 已核实事实 | 环境与时间 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- | --- |
| 来源控制模型 | 新 migration `e951b2d0a6c4` 定义来源身份/版本、收件箱、规则/规则版本、映射异常与预览运行。相同来源键与哈希幂等、哈希变化冲突；已发布规则版本只能退役。 | 本地代码与迁移审阅 | 本地代码 | 未在恢复副本或生产执行。 |
| 应用权限与接口 | `fin_app` 被撤销来源控制表的写权限；V2 仅有来源收件箱和已持久化 preview 的 GET 接口，前端无来源写入、自动草稿、自动审核或自动过账操作。 | 本地测试与构建 | 本地测试 | 没有受控导入身份时，收件箱空是安全状态。 |
| 本地验证 | 规则范围/有效期/冲突、来源哈希幂等/冲突、模型与只读路由测试均通过；完整定向套件、类型检查与生产构建仍须以本次最终回归输出为准。 | 本地，2026-07-30 | 本地测试 | 不替代恢复副本迁移、真实来源快照导入或生产验收。 |

**保持 No-Go：** 来源导入身份、华邦数据的结算/税务/法人/账簿事实与规则映射尚未具备。V2.0 继续只允许 dry-run/preview，不创建生产草稿。

## 增量迁移恢复副本演练入口（2026-07-30，本地）

| 项目 | 已核实事实 | 环境与时间 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- | --- |
| 静态迁移链 | 从生产现有 head `1fdf4577d7d8` 到本地 head `e951b2d0a6c4` 的 Alembic 静态 SQL 成功生成，覆盖期初、报表和来源收件箱三项迁移。全库从零版本静态渲染会被一个既有、依赖业务数据的历史迁移阻断，因此不以该命令判断增量发布。 | 本地，2026-07-30 | 本地迁移预检 | 增量 SQL 可审阅；不替代 PostgreSQL 实际执行。 |
| 受限演练脚本 | 新增 `deploy/rehearse_finance_v2_migrations.sh`：仅接受名称符合 `huabang_ai_finance_drill_*` 的既有恢复库；生产 `huabang_ai` 被拒绝。执行模式固定为 baseline→head→baseline→head，并执行只读角色核验；不含服务重启或 stdin sudo。 | 本地 TDD，2026-07-30 | 本地测试 | 为恢复副本验证建立受限入口，不改变生产。 |
| 实际演练与独立复核 | 项目管理员通过 SSH TTY 在 `huabang_ai_finance_drill_20260729_r2` 完成 `1fdf4577d7d8 → 016cfd3c1454 → c82e5a1f9d70 → e951b2d0a6c4 → 1fdf4577d7d8 → e951b2d0a6c4`。两次角色核验均为 `status=ready`。随后以 `fin_app` 独立只读探针核验：历史凭证 339、分录 4,596、当前凭证 0；期初/报表/来源表均存在，来源表 INSERT 为 false。 | 隔离恢复副本，2026-07-30 | 恢复演练与独立只读验证 | 三项迁移与最小权限恢复 Gate 通过；不等同于生产迁移、应用切流或生产开写。 |

**下一 Gate：** 核实生产只读发布的不可变输入、备份与回滚点；仅以受控发布入口执行生产迁移与初始只读发布。恢复成功不授权开启 V2 草稿、审核、过账或来源写入。

## 生产只读发布预检（2026-07-30）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 生产工作树差异 | 运行工作树当前提交 `a52a679` 与财务功能分支共同基线为 `bcdc1e9`；差异只包含旧发布遗留的 `.finance-v2-*`、环境备份与旧前端产物，没有新增源码。该残留不并入功能分支。 | 生产 Git 只读核验 | 受控发布可保留 `a52a679` 作为回滚点；不得合并残留资产。 |
| 不可变历史输入 | 服务器清单 `/home/xiaohu/kingdee_imports/K3MIG_20260717_172928/canonical_import/manifest.json` 的 SHA-256 为 `390a3033a00e32cb03534bbf9bb01fc615c0b20c3a1131bba0532a68e4b7b195`。 | 生产文件只读核验 | 作为初始只读发布的历史导入输入。 |
| 发布无写预检 | `release_finance_center_v2.sh` 对功能分支 `33d3756d5b6d0df6e6239217665988a5fc7f1706`、该清单哈希与预期历史 339/4,596 返回成功的 dry preflight。 | 生产发布入口预检 | 输入约束通过；尚未执行备份、迁移、重启或应用切换。 |

**下一 Gate：** 仅由受控 SSH TTY 执行同一发布入口的 `--execute`，保存生产备份、迁移、服务健康和只读验证的原始结果。发布后仍保持所有 V2 写 Gate 关闭。

## 生产只读发布执行前状态复核（2026-07-30，后补）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 当前运行态 | 生产目录当前提交为 `a52a679894d6e533d87efcb3d79e81b43f696793`；`huabang-backend.service` 为 `active`；`/health` 显示数据库和 Redis 已连接。该提交不是待发布的功能提交 `cd47fbb110a071d58e63a8038b15d40fe84bf3df`。 | 生产 SSH 只读核验 | 恢复副本演练不等于生产只读发布；不得据此开启任何 V2 写 Gate。 |
| 发布互斥锁 | `.finance-v2-release.lock` 为空文件，最后修改于 `2026-07-29 12:12:19 UTC`；未发现运行中的 `release_finance_center_v2.sh` 或恢复演练脚本进程。发布脚本以 `flock` 获取文件描述符锁，因此这个无持锁进程的遗留文件本身不阻断下一次受控发布。 | 生产 SSH 只读核验与发布脚本审阅 | 不清理、不修改遗留文件；由受控发布入口自行获取锁。 |
| 当前可执行输入 | 当前功能分支头为 `a8ede7058504cf3614ec9a71d572bc93a30d1550`；历史清单仍为 SHA-256 `390a3033a00e32cb03534bbf9bb01fc615c0b20c3a1131bba0532a68e4b7b195`，预期 339 张凭证、4,596 条分录。生产发布入口已对这一组输入完成无写 dry preflight。 | 本地 Git 与生产文件只读核验、生产发布入口预检 | 必须以这一组输入执行受控 `--execute`，不能复用较早 `33d3756` 的预检结论。 |

**当前 Go / No-Go：** 恢复副本迁移 Gate 为 **Go**；生产只读发布尚未执行，且因需要项目管理员在 SSH TTY 提供 sudo 授权而保持待执行。即使该发布完成，`draft_enabled`、`review_enabled`、`post_enabled`、`period_close_enabled` 与来源写入仍保持 **No-Go**。

## 生产只读发布中断与输入修复（2026-07-30）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 中断位置 | 受控发布完成备份、角色引导、三项 V2 迁移、数据库角色核验和权限种子后，在历史导入前的来源校验因 `AIS20251127140257: missing database snapshot metadata` 退出。未执行该次历史暂存或发布。 | 项目管理员发布输出与生产只读复核 | 不可将本次中断表述为生产只读发布成功。 |
| 运行时回退状态 | trap 已将代码/配置/前端恢复至 `a52a679`，但保留 `e951b2d0a6c4` schema；旧代码运行 `alembic current` 无法识别该 revision，旧 `.env` 中的 `fin_app` 登录也无法认证。 | 生产 SSH 只读核验 | 当前代码—schema—财务会话不一致；不得开启任何 Gate，须以受控 V2 运行时重新发布修复。 |
| 输入根因 | 服务器误用了 8,194 字节的 `canonical_import/manifest.json`，其中 `account_sets` 标记三账套为 official、但 `databases=[]` 且没有 `databases/` 目录。它不是导入器要求的原生快照根。 | 生产文件只读核验 | 输入 SHA 正确不代表语义完整；发布前必须执行导入器 dry-run。 |
| 已验证替代输入 | 本地原生根 `D:\huabang\invest_kingdee\results\K3MIG_20260717_172928\manifest.json` 的 SHA-256 为 `BBE9676545C413A5AE077796D766C04E572A75D1F918B1053FA9548F1D8DB1FD`。服务器新候选目录仅补齐导入所需的 9 个哈希校验表文件；服务器 dry-run 返回 339 凭证、4,596 分录、0 冲突。未覆盖旧 canonical 输入、未写财务表。 | 本地与生产只读 dry-run | 可作为下一次受控发布的唯一历史输入；仍须完成运行时修复与发布后独立核验。 |
| 发布脚本修复 | 新回归要求历史导入 dry-run 在 `--execute`、`sudo_init`、备份和迁移之前运行；错误输入必须在任何特权或数据库变更前失败。 | 本地 TDD | 修复需经测试、审阅、推送后随运行时恢复发布。 |
...
## 发布后回归与开写前外部 Gate 复核（2026-07-30）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 本地财务回归 | 以无敏感进程级测试配置运行当前工作树实际 `test_finance_v2_*.py`、历史导入与数据库角色定向套件，结果 `148 passed`。 | 本地测试 | 代码回归通过；不替代恢复副本或生产开写。 |
| 前端回归 | `vue-tsc --noEmit` 与 Vite production build 通过，构建产出 Finance V2 核心工作台。仅有既有 Vite CJS / Rollup 注释警告。 | 本地构建 | 前端可构建；不替代已登录浏览器验收。 |
| 已登录浏览器验收 | 可控 Chrome 访问财务工作台被重定向至 `/login`，未输入凭据、验证码或切换账号。 | 浏览器运行时 | 未形成财务角色或超级管理员的端到端验收，继续 No-Go。 |
| 通知闭环 | 钉钉客户端凭据与 stream 服务存在，但运行健康显示全局推送关闭，未发现 `finance-v2-oncall` 接收人配置。 | 生产只读配置与运行时 | 不自行启用或发送告警；真实告警送达/确认/关闭继续 No-Go。 |
| 异机恢复 | 逻辑备份和物理基线 timer 最近一次均成功，但备份目录与生产数据同属服务器根文件系统。 | 生产运行时 | 本机 PITR 证据不等于主机损坏场景的 RPO/RTO，继续 No-Go。 |

## 实施计划—当前工作树路径审计（2026-07-30）

| 项目 | 已核实事实 | 结论 |
| --- | --- | --- |
| 计划路径清单 | `implementation-plan.md` 列出 76 个 Create/Modify 路径；当前工作树中 46 个同路径存在、30 个不存在。 | 不得以计划文件名直接证明实现完成。 |
...
## 凭证入口 V2 Gate 收敛（2026-07-30，本地）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 路由漂移 | 财务中心 `/app/finance-center/vouchers` 的配置声称使用 `fin_current`，但组件实际渲染旧 `FormalLedger.vue`，该页面调用旧 `financeCenterApi` 写接口。旧页面仍供旧财务路由使用，但不能作为 V2 菜单实现。 | 当前工作树调用链审计 | 只读 V2 菜单不应绕过 V2 后端 Gate。 |
| 纠正 | V2 “凭证”菜单改为渲染 `V2CoreWorkspace.vue`，其所有写动作依赖 `/api/v1/finance-center/v2` 后端 Gate；旧 `FormalLedger.vue` 未删除、未改写，也未关闭旧系统正式写路径。 | 失败测试后的最小代码修正 | 历史只读期继续保留旧系统唯一正式写账语义，同时避免 V2 菜单触达旧写接口。 |
| 本地验证 | 新入口回归先失败再通过；前端静态回归 `15 passed`、`vue-tsc --noEmit`、Vite build 通过。代码审阅确认未新增依赖、未改变权限模型、未增加新的写操作或开关。提交 `4be270b` 仅在本地，仍受 SSH 链路阻塞。 | 本地 TDD、构建与代码审阅 | 必须在生产发布后用已登录用户复验菜单数据源和 Gate 提示。 |

## 账簿与报表 V2 菜单隔离（2026-07-30，本地）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 遗留链接 | 财务中心的“账簿”和“报表”模块内嵌链接仍指向旧 `/app/fin/history/*` 和 `/app/fin/formal-ledger`，可使 V2 菜单回到旧数据域。 | 当前工作树调用链审计 | V2 菜单完整性不成立，不能作为最终切换依据。 |
| 纠正 | 两个模块改为使用同一受 V2 Gate 保护的工作台；移除了其 V2 页面中的所有旧 `/app/fin/*` 链接。旧路由保持原状，仍仅由旧系统入口使用。 | 失败测试后的最小代码修正 | 不关闭旧系统，不改变历史只读或任何 V2 写入开关。 |
| 本地验证 | 新增回归先失败后通过；前端静态回归 `16 passed`、`vue-tsc --noEmit` 与 Vite build 通过。提交 `7c774e1` 仅在本地，待 SSH 恢复后连同前序修正一起受控发布。 | 本地 TDD、构建与代码审阅 | 发布后仍须浏览器验收，生产状态未变。 |

## V2 API 响应解包纠正（2026-07-30，本地）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 浏览器症状与接口事实 | 已登录工作台将监控消息显示为 `success`、策略表为空；同一会话只读调用 `/api/v1/finance-center/v2/monitoring/summary` 返回 HTTP 200、11 条策略和 3 项持久化指标。 | 已登录浏览器与受保护 API | 证明并非权限、监控数据或数据库迁移缺失。 |
| 根因 | `financeV2Api` 将 Axios `ApiResponse` 外层直接交给工作台，页面把外层 `message=success` 和外层对象误当业务数据。该问题同样影响 V2 账簿、历史、来源和写入命令响应的消费。 | 前端请求拦截器与调用链审阅 | 只读验收页面不能正确显示已发布 V2 事实，需修正客户端边界。 |
| 纠正与验证 | V2 专属客户端统一解包 `ApiResponse.data`，保持对现有工作台消费者的 `{ data: T }` 接口；不修改全局 Axios 拦截器，因其会影响旧 API。回归先失败后通过；前端静态回归 `17 passed`、`vue-tsc --noEmit` 与 Vite build 通过。提交 `18385bb` 仅在本地。 | 本地 TDD、类型检查、构建与代码审阅 | 必须发布后以同一已登录会话验证 11 条策略与历史数据真实渲染；不影响 V2 写 Gate。 |

## V2 API 双层解包兼容修复（2026-07-30，本地）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 发布后浏览器症状 | 已登录浏览器在发布 `11f4850` 后，V2 监控与历史接口均返回 HTTP 200；监控 API 含 11 条策略、历史 API 含已发布记录，但页面未渲染数据。 | 已登录浏览器与受保护 API | 生产只读数据仍正确，前端展示验收尚未通过。 |
| 根因 | 运行时请求边界可返回完整 `ApiResponse`，也可返回已解包 payload；此前的 V2 专属解包器只接受前者，遇到后者再次读取 `.data` 得到 `undefined`。 | 浏览器网络事实、已部署 bundle 与调用链排查 | 不涉及数据库、历史数据或写 Gate。 |
| 最小修复 | `unwrapFinanceV2` 仅在对象同时具有 `code`、`success`、`data` 时才拆开平台信封；否则原样保留 payload。避免未来业务 payload 恰有 `data` 字段时被误拆。 | 本地 TDD 与代码审阅 | 保持既有 V2 消费者 `{ data: T }` 契约，不改变全局 Axios 或旧财务页面。 |
| 本地验证 | 新回归先失败后通过；前端静态回归 `18 passed`，`vue-tsc --noEmit` 和 Vite build 均通过。 | 本地测试与构建 | 仍需受控发布和已登录浏览器复验；所有 V2 写 Gate 继续 No-Go。 |
