...
| 本地迁移图 | `bda75497f94f` 是唯一 head；该 revision 于 2026-07-29 从实际图生成，增加 `voucher_line` 借贷不可同时为正约束；实施计划中不得预写未来 revision ID | 本地，2026-07-29 | 本地只读 | 新迁移必须从当天实际图生成并在恢复副本验证 |
...
| 后端 V2 定向测试 | 在临时非生产 `APP_SECRET_KEY`/`DB_PASSWORD`/`JWT_SECRET_KEY` 环境变量下，11 个 `test_finance_v2*.py` 文件及角色核验共 31 项通过 | 本地，2026-07-29 | 本地测试 | 可继续实现；不是数据库恢复或生产验收 |
