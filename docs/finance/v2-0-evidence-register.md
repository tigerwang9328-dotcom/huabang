...
## V2 只读发布修复后的生产浏览器验收（2026-07-31）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 已发布运行版本 | 生产提交为 `5257d400ee75755dd334295631682abd102bcf78`；`huabang-backend.service` 为 `active`。同环境脚本在加载受管 `.env` 后返回 `status=ready`，历史凭证 339、历史分录 4,596、当前凭证 0、启用写 Gate 为空。 | 生产 SSH 只读核验 | 证明 API、数据库和服务处于一致的只读发布状态；不授权写入。 |
| 已登录工作台验收 | 受控 Chrome 的新建已登录标签访问 `/app/finance-center/core-workspace`。页面显示“当前为只读验收阶段”；异步加载完成后，监控表渲染 11 项策略，历史档案渲染首批 100 张凭证及“历史数据”标记。 | 生产浏览器运行时 | 已验证发布修复后的真实页面消费 V2 受保护接口，不再出现将外层 `message=success` 作为业务数据的症状。 |
| 受保护接口交叉核验 | 同一已登录会话调用监控摘要和历史凭证接口均为 HTTP 200；前者返回 11 项策略，后者按 `limit=100` 返回 100 条历史记录。 | 生产浏览器运行时 | 页面渲染与接口事实一致；历史数据仍只读。 |
| 写入口负向验收 | 页面未出现保存草稿、审核或人工过账按钮；当前账为空时只显示“尚未建立 V2 当前账账簿”，未误报为零余额或可制单状态。验收未点击任何写入动作。 | 生产浏览器运行时 | `draft_enabled`、`review_enabled`、`post_enabled`、`period_close_enabled` 与来源写入继续 **No-Go**。 |

**当前 Go / No-Go：** 生产只读发布、已登录浏览器展示、历史标记与 V2 菜单隔离均为 **Go**。正式开写仍为 **No-Go**：异机恢复/RPO-RTO 演练、告警真实送达与关闭闭环、非空生产形态性能、最终期初、获批期边冻结窗口、旧写入口切换以及人工凭证试点均未形成可替代的生产证据。

## 开写前备份与异机恢复复核（2026-07-31）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 定时逻辑备份 | `huabang-postgres-backup.timer` 已启用；最近一次逻辑备份服务于 `2026-07-30 15:01:55–15:02:08 UTC` 成功退出，产物记录为 `/var/backups/huabang-postgres/huabang_ai_20260730T150155Z.dump`。 | 生产 systemd 只读核验 | 可作为本机逻辑恢复输入，但未证明恢复成功。 |
| 定时物理基线备份 | `huabang-postgres-basebackup.timer` 已启用；最近一次于 `2026-07-31 01:33:59–01:34:58 UTC` 成功退出，产物记录为 `/var/backups/huabang-postgres/base/base_20260731T013359Z`。 | 生产 systemd 只读核验 | 存在本机物理基线，但尚未在独立环境恢复并测量 RTO。 |
| 故障域隔离 | `/` 与 `/var/backups` 同为 `/dev/mapper/ubuntu--vg-ubuntu--lv` 的 ext4 文件系统。当前未核实任何异机/异存储副本，也未形成主机损坏恢复演练。 | 生产挂载点只读核验 | 不满足主机损坏场景的 RPO≤24 小时、RTO≤4 小时证据要求；正式开写维持 **No-Go**。 |

**需要的外部事实：** 一个已授权、可验证的异机备份目标或隔离恢复副本环境。系统可在获得该目标后执行不含财务写入的恢复演练、记录恢复耗时并核对 339/4,596 历史数据；在此之前不得将本机备份成功描述为灾备 Gate 通过。

## 开写前告警通知闭环复核（2026-07-31）

| 项目 | 已核实事实 | 证据层级 | Gate 影响 |
| --- | --- | --- | --- |
| 现有钉钉运行形态 | `huabang-dingtalk-stream.service` 已启用且运行中；其定义和运行代码说明它是 Stream 入站事件消费者。 | 生产 systemd 与代码只读核验 | 服务运行不证明 Finance V2 告警可发送、可确认或可关闭。 |
| Finance V2 指标策略 | 全部 11 项 Finance V2 指标策略的通知路由均为逻辑名称 `finance-v2-oncall`，且 `notification_configured=False`；当前实现仅安全公开脱敏指标策略，不包含向钉钉或其他外部系统投递 Finance V2 告警的适配器。 | 当前代码、定向测试与生产页面 | 指标可见不等于告警闭环；不得将未接入的指标以 0 代替。 |
| 未执行的动作 | 未读取或输出任何钉钉凭据，未开启 `DINGTALK_PUSH_ENABLED`，未发送测试通知，也未创建或猜测 `finance-v2-oncall` 接收人。 | 本次生产只读核验 | 保持现有外部消息边界；正式开写维持 **No-Go**。 |

**需要的外部事实：** 获批的告警接收群/值班人、通知方式及可用于验证送达与关闭的测试窗口。V2.0 只可在该信息明确后复用已有中台通知能力或采用经批准的独立运维路由；不得把它扩展为百胜/钉钉业务来源正式适配。
