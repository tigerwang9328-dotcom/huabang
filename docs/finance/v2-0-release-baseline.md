...
| 本地 Alembic | 单一 head `232ec287c52f`（父 `354401508750`）；含凭证明细约束、凭证编号审计表、`operation_event` 追加写保护、已发布历史分录视图/索引以及结账批次与双人反结账审批表 | 已核实，尚未在恢复副本或生产执行 |
...
| 本地后端验证 | V2 定向测试、角色核验和历史导入 dry-run 78 passed（临时非生产配置）；含职责分离、独立过账尝试审计、凭证命令幂等、锁定式编号、结账中人工过账阻断、结账检查/双人反结账、历史分录下钻、当前账试算表与监控摘要 | 已核实 |
| 本地前端验证 | 财务中心入口、服务端 Gate 就绪状态、受控草稿/命令工作台、历史金蝶分录下钻、结账检查与当前账试算表回归，类型检查与 Vite 构建成功 | 已核实 |
