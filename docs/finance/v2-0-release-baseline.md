...
| 本地 Alembic | 单一 head `354401508750`（父 `7c05884df060`）；含凭证明细约束、凭证编号审计表、`operation_event` 数据库追加写保护，以及仅呈现已发布历史分录的 `fin_read.history_voucher_line` 视图和查询索引 | 已核实，尚未在恢复副本或生产执行 |
...
| 本地后端验证 | V2 定向测试、角色核验和历史导入 dry-run 69 passed（临时非生产配置）；含职责分离、独立过账尝试审计、凭证命令幂等、锁定式凭证编号、审计不可篡改迁移、期间结账阻断、金蝶来源哈希/红字校验、暂存发布计划及已发布历史分录只读下钻 | 已核实 |
| 本地前端验证 | 财务中心入口、服务端 Gate 就绪状态、受控草稿/命令工作台与历史金蝶凭证/分录只读下钻回归，类型检查与 Vite 构建成功 | 已核实 |
