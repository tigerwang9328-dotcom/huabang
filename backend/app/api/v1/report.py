...
async def list_boss_daily(
    ...
    r = await db.execute(text("""
        SELECT report_date, total_sales, actual_pay_amount, order_count,
               ...
               avg_discount_rate, return_rate, deposit_amount,
    ...
    items = [{
        ...
        "deposit_amount": float(row[9]) if row[9] is not None else None,
        "gross_profit": float(row[10]) if row[10] is not None else None,
        "gross_margin": float(row[11]) if row[11] is not None else None,
        "vip_sales_amount": float(row[12]) if row[12] is not None else None,
        "vip_negative_balance_amount": float(row[13]) if row[13] is not None else None,
        "is_cost_complete": bool(row[14]),
        "data_quality_status": row[15] or "warning",
        "metric_status": row[16] or {},
        "has_ai_summary": bool(row[17]),
        "generated_at": str(row[18]) if row[18] else None,
