...
class ReportService:
    async def generate_boss_daily(self, stat_date: str, db: AsyncSession, force: bool = False) -> dict:
        ...
        # 生成AI摘要
        ai_result = await self._generate_ai_summary(report_data, db, allow_model=False)
    ...
    async def _get_existing_report(self, stat_date: str, db: AsyncSession) -> dict:
        ...
        r = await db.execute(text("""
            SELECT report_date, total_sales, offline_sales, online_sales, online_ratio,
                   ...
                   actual_pay_amount, return_amount, return_rate, deposit_amount,
        ...
        report = {
            ...
            "gross_margin": float(row[13]) if row[13] is not None else None,
            ...
            "deposit_amount": float(row[35] or 0),
            "inventory_total_qty": float(row[36] or 0),
            "vip_balance": float(row[37] or 0),
            "vip_negative_balance_count": int(row[38] or 0),
            "vip_sales_amount": float(row[39] or 0),
            "vip_sales_ratio": float(row[40] or 0),
            "major_exception_count": int(row[41] or 0),
            "source_freshness": row[42] or {},
            "metric_status": row[43] or {},
            "inventory_age_unknown_qty": float(row[44] or 0),
            "inventory_age_unknown_amount": float(row[45] or 0),
            "command_conclusion": row[46] or None,
