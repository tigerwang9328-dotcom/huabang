...
@pytest.mark.asyncio
async def test_create_voucher_converts_duplicate_number_to_conflict(monkeypatch):
    from types import SimpleNamespace
    from sqlalchemy.exc import IntegrityError
    from fastapi import HTTPException
    from app.api.v1 import mumaren_finance_center as finance_api
    from app.api.v1.mumaren_finance_center import VoucherCreateInput

    async def duplicate(*_args, **_kwargs):
        raise IntegrityError("insert", {}, Exception("uq_mumaren_finance_voucher_no"))
    ...
    monkeypatch.setattr(finance_api, "create_mumaren_voucher", duplicate)
    with pytest.raises(HTTPException) as exc:
        await finance_api.create_voucher(
            body=VoucherCreateInput(
                book_id=1,
                voucher_no="1",
                voucher_date="2026-08-02",
                lines=[
                    {"account_id": 1, "debit_amount": "1", "credit_amount": "0"},
                    {"account_id": 2, "debit_amount": "0", "credit_amount": "1"},
                ],
            ),
    ...
    assert "相同凭证号" in exc.value.detail
