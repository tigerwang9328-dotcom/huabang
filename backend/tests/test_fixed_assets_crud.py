...
def test_fixed_asset_update_accepts_the_dispose_payload_sent_by_the_ui():
    """The UI sends the selected book and the target disposal status in the body."""
    body = FixedAssetUpdate.model_validate({"book_id": 1, "status": "disposed"})
    assert body.status == "disposed"


@pytest.mark.asyncio
async def test_update_fixed_asset_marks_a_same_book_asset_disposed():
    asset = _asset(asset_id=1, book_id=1, status="draft")
    db = _MockDb(get_map={FinanceCenterMumarenFixedAsset: {1: asset}})
    result = await update_fixed_asset(
        asset_id=1,
        body=FixedAssetUpdate.model_validate({"book_id": 1, "status": "disposed"}),
        current_user=_FakeUser(user_id=8),
        db=db,
    )
    assert result.data["status"] == "disposed"
