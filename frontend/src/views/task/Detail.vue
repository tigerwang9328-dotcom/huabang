      ...
      <div v-if="task">
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="状态">
            <el-tag :type="statusType[task.status] || \"info\"">{{ task.status }}</el-tag>
          ...
          <el-descriptions-item label="风险等级">{{ task.risk_level }}</el-descriptions-item>
          <el-descriptions-item label="问题描述" :span="2">{{ task.description || "--" }}</el-descriptions-item>
          <el-descriptions-item label="数据依据" :span="2">{{ task.data_evidence_text || JSON.stringify(task.data_evidence) || "--" }}</el-descriptions-item>
          <el-descriptions-item label="反馈要求" :span="2">{{ task.feedback_requirement || "--" }}</el-descriptions-item>
          <el-descriptions-item label="复查指标" :span="2">{{ JSON.stringify(task.review_metrics) || "--" }}</el-descriptions-item>
          <el-descriptions-item label="需人工确认">{{ task.requires_human_confirm ? "是" : "否" }}</el-descriptions-item>
