  ...
  <section class="panel">
    ...
    <el-table v-loading="loading" :data="assets" empty-text="暂无资产卡片" stripe show-summary :summary-method="summary">
      ...
      <el-table-column label="净值" width="140" align="right">
        <template #default="{ row }">{{ money(Number(row.original_value || 0) - Number(row.accumulated_depreciation || 0)) }}</template>
      </el-table-column>
      <el-table-column prop="purchase_date" label="购入日期" width="130" />
      <el-table-column label="状态" width="110">
        <template #default="{ row }">
          <el-tag :type="row.status === 'disposed' ? 'info' : 'success'" size="small">{{ row.status === 'disposed' ? '已处置' : '在用' }}</el-tag>
        </template>
      </el-table-column>
...
const summary = ({ columns, data }: { columns: any[]; data: MumarenFixedAsset[] }) => {
  ...
  if (idxNet >= 0) sums[idxNet] = money(data.reduce((s, a) => s + Number(a.original_value || 0) - Number(a.accumulated_depreciation || 0), 0));
  return sums;
};
</script>
