  ...
  <section class="panel">
    <div class="heading">
      <div>
        ...
        <h2>应付明细</h2>
        <p>{{ isReadonly ? "金蝶迁移账簿未导入应付业务单据与账龄；请通过原始凭证、明细账和余额快照核对。" : "按供应商展开应付明细与账龄分桶。" }}</p>
      </div>
    ...
    <el-alert v-else-if="isReadonly" type="warning" title="金蝶迁移账簿未导入应付业务单据与账龄，当前页面不以零余额表示历史应付。" :closable="false" show-icon />
    ...
    <el-empty v-else-if="!error && !isReadonly" description="请选择独立账簿后查询应付明细" />
  </section>
</template>
...
const load = async () => {
  ...
  error.value = "";
  try {
    const response = await mumarenFinanceCenterApi.getArApAging({ book_id: requestedBookId, order_type: "payable" });
