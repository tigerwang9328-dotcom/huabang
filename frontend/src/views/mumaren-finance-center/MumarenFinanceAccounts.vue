...
const form = reactive({ account_code: "", account_name: "", account_type: "asset" as "asset" | "liability" | "equity" | "income" | "expense", direction: "debit" as "debit" | "credit", level: 1 });
...
const openCreate = () => {
  ...
  Object.assign(form, { account_code: "", account_name: "", account_type: "asset", direction: "debit", level: 1 });
...
const save = async () => {
  ...
  saving.value = true;
  try {
    if (editing.value) await mumarenFinanceCenterApi.updateAccount(bookId.value, editing.value.id, { book_id: bookId.value, account_name: form.account_name, account_type: form.account_type, direction: form.direction, level: form.level });
    else await mumarenFinanceCenterApi.createAccount(bookId.value, { ...form });
