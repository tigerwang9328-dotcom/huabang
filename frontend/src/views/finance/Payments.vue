<template>
  <div class="page">
    <div class="page-header"><h2 class="page-title">付款申请</h2></div>
    <div class="sec" v-loading="loading">
      <el-empty v-if="!items.length && !loading" description="暂无付款申请，待钉钉付款审批数据接入后展示" :image-size="90" />
      <el-table v-else :data="items" size="small" stripe>
        ...
        <el-table-column prop="expense_type" label="付款事项" min-width="150" />
        ...
        <el-table-column label="付款状态" width="100"><template #default="{row}"><el-tag size="small" :type="row.payment_status==='paid'?'success':'info'">{{ row.payment_status==='paid'?'已付款':'未付款' }}</el-tag></template></el-table-column>
...
const load = async () => {
  loading.value = true;
  try { const res = await financeApi.getPayments({ page: page.value, page_size: pageSize }); const d = res.data.data || {}; items.value = d.items || []; total.value = d.total || 0; }
