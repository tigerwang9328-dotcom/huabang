...
async function saveStyle() { if (!context.value || !styleForm.value.style_code.trim() || !styleForm.value.style_name.trim()) { ElMessage.error("请填写款号和名称"); return; } saving.value = true; try { await douyinColorAnalyticsApi.createStyle({ account_id: context.value.account.id, style_code: styleForm.value.style_code.trim(), style_name: styleForm.value.style_name.trim() }); await load(); styleForm.value = { style_code: "", style_name: "" }; styleDialog.value = false; ElMessage.success("已保存款号，服务端已记录操作"); } catch (error) { loadError.value = describeError(error); } finally { saving.value = false; } }
async function saveColor() { if (!context.value || !selectedStyle.value || !colorForm.value.color_code.trim() || !colorForm.value.color_name.trim()) { ElMessage.error("请填写颜色编码和名称"); return; } saving.value = true; try { await douyinColorAnalyticsApi.createColor(selectedStyle.value.id, { account_id: context.value.account.id, color_code: colorForm.value.color_code.trim(), color_name: colorForm.value.color_name.trim() }); await showColors(selectedStyle.value); colorForm.value = { color_code: "", color_name: "" }; ElMessage.success("已保存颜色，服务端已记录操作"); } catch (error) { loadError.value = describeError(error); } finally { saving.value = false; } }
