<template>
  <section class="annotation-page" v-loading="loading">
    ...
    <el-alert v-else title="一件主要衣物规则" description="clear_primary 必须选择一件款号和一种颜色；multi_focus、unclear 会清空款色且只记录质量状态，不参与颜色排名。" type="warning" show-icon :closable="false" class="banner" />
        ...
        <el-form label-position="top">
          ...
          <el-form-item label="主要分析衣物判断">
            <el-radio-group v-model="editor.focus_status">
              <el-radio value="clear_primary">唯一主要衣物</el-radio>
              <el-radio value="multi_focus">多件重点，不进入颜色排名</el-radio>
              <el-radio value="unclear">无法判断，不进入颜色排名</el-radio>
            </el-radio-group>
          </el-form-item>
          <template v-if="editor.focus_status === 'clear_primary'">
            <el-form-item label="主要款号（一个片段只能指定一件主要分析衣物）">
              <el-select v-model="selectedStyleId" placeholder="选择款号" filterable class="full-width">
            ...
            <el-form-item label="主要颜色">
              <el-select v-model="selectedColorId" placeholder="先选择款号" :disabled="!selectedStyleId" filterable class="full-width">
                <el-option v-for="color in colors" :key="color.id" :label="`${color.color_code} · ${color.color_name}`" :value="color.id" />
              </el-select>
            </el-form-item>
          </template>
          <el-form-item label="标注说明（可选）"><el-input v-model="editor.focus_note" type="textarea" :rows="3" maxlength="500" show-word-limit placeholder="记录主要衣物判断、切换原因或审核需要关注的事实" /></el-form-item>
        </el-form>
...
const colors = ref<DouyinGarmentColor[]>([]);

const newEditor = () => ({ id: null as number | null, version: 0, input_start_ms: 0, input_end_ms: 1_000, focus_status: "clear_primary" as DouyinFocusStatus, style_id: null as number | null, color_id: null as number | null, focus_note: "" });
...
const selectedStyleId = computed({ get: () => editor.value.style_id, set: (value: number | null) => { editor.value.style_id = value; editor.value.color_id = null; } });
const selectedColorId = computed({ get: () => editor.value.color_id, set: (value: number | null) => { editor.value.color_id = value; } });
...
watch(() => editor.value.focus_status, (status) => { if (status !== "clear_primary") clearNonPrimaryGarment(); });
watch(selectedStyleId, async (styleId) => {
  colors.value = [];
  if (!styleId || !context.value) return;
  try { colors.value = (await douyinColorAnalyticsApi.listColors(styleId, context.value.account.id)).data.items || []; } catch (error) { ElMessage.error(describeError(error)); }
});
...
function clearNonPrimaryGarment() { editor.value.style_id = null; editor.value.color_id = null; }
function startNew() { validationErrors.value = []; editor.value = newEditor(); }
async function editClip(clip: DouyinVideoClip) {
  editor.value = { id: clip.id, version: clip.version, input_start_ms: clip.input_start_ms, input_end_ms: clip.input_end_ms, focus_status: clip.focus_status, style_id: clip.style_id, color_id: clip.color_id, focus_note: clip.focus_note || "" };
  if (clip.style_id && context.value) colors.value = (await douyinColorAnalyticsApi.listColors(clip.style_id, context.value.account.id)).data.items || [];
}
...
function validate() {
  ...
  if (editor.value.focus_status === "clear_primary" && (!editor.value.style_id || !editor.value.color_id)) errors.push("唯一主要衣物必须同时选择款号和颜色");
  if (editor.value.focus_status !== "clear_primary" && (editor.value.style_id || editor.value.color_id)) errors.push("非唯一主要衣物不得携带款号或颜色");
...
async function save() {
  ...
  saving.value = true;
  try {
    const payload = { account_id: context.value.account.id, video_id: video.value.id, input_start_ms: editor.value.input_start_ms, input_end_ms: editor.value.input_end_ms, curve_resolution_ms: curveResolutionMs(), focus_status: editor.value.focus_status, style_id: editor.value.focus_status === "clear_primary" ? editor.value.style_id : null, color_id: editor.value.focus_status === "clear_primary" ? editor.value.color_id : null, focus_note: editor.value.focus_note || null };
...
function focusLabel(status: DouyinFocusStatus) { return { clear_primary: "唯一主要衣物", multi_focus: "多件重点", unclear: "无法判断" }[status]; }
...
@media (max-width: 900px) { .annotation-page { padding: 16px; }.page-head, .workspace { display: block; }.head-actions { margin-top: 12px; }.editor-panel { margin-top: 16px; } }
