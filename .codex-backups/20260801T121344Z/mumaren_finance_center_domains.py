...
from app.services.mumaren_finance_center.workflow import create_voucher as create_mumaren_voucher
...
def _period_dates(period_code: str) -> tuple[date, date]:
    try:
        start = datetime.strptime(period_code, "%Y-%m").date().replace(day=1)
    except ValueError as error:
        raise HTTPException(status_code=422, detail="期间必须是 YYYY-MM 格式") from error
    if start.month == 12:
...
async def initialize_fiscal_period(
    ...
    created = FinanceCenterMumarenFiscalPeriod(
        book_id=book_id, period_code=period, start_date=start_date, end_date=end_date,
        status="open", is_readonly=False,
    )
    db.add(created)
    await db.flush()
    _add_audit_log(db, book_id=book_id, action="initialize_period", operator_id=_actor_id(current_user), detail=period)
    await db.flush()
    return ApiResponse.ok(data=_fiscal_period_data(created), message="会计期间已初始化")
...
async def precheck_fiscal_period(
    book_id: int = Query(ge=1),
    period: str = Query(pattern=r"^\d{4}-\d{2}$"),
    current_user: SysUser = Depends(require_mumaren_finance_access),
    db: AsyncSession = Depends(get_db),
):
    """执行结账前预检；error 阻断结账，warning 只提示。"""
    start_date, end_date = _period_dates(period)
    ...
    if period_row is None:
        checks.append({"key": "period_initialized", "title": "会计期间已初始化", "severity": "error", "passed": False, "count": 1, "message": "请先初始化该会计期间。"})
    else:
        checks.append({"key": "period_initialized", "title": "会计期间已初始化", "severity": "success", "passed": True, "count": 0, "message": "会计期间已初始化。"})
        if period_row.status != "open":
            checks.append({"key": "period_open", "title": "期间状态可结账", "severity": "error", "passed": False, "count": 1, "message": f"当前状态为 {period_row.status}，不能结账。"})
        else:
            checks.append({"key": "period_open", "title": "期间状态可结账", "severity": "success", "passed": True, "count": 0, "message": "期间处于开放状态。"})

    unposted = await _count(db, select(func.count()).select_from(FinanceCenterMumarenVoucher).where(
        FinanceCenterMumarenVoucher.book_id == book_id,
        FinanceCenterMumarenVoucher.voucher_date >= start_date,
        FinanceCenterMumarenVoucher.voucher_date <= end_date,
        FinanceCenterMumarenVoucher.status != "posted",
    ))
    checks.append({"key": "unposted_vouchers", "title": "凭证已全部过账", "severity": "error" if unposted else "success", "passed": unposted == 0, "count": unposted, "message": "所有凭证均已过账。" if not unposted else f"本期有 {unposted} 张凭证未过账。"})

    unbalanced = await _count(db, select(func.count()).select_from(FinanceCenterMumarenVoucher).where(
        FinanceCenterMumarenVoucher.book_id == book_id,
        FinanceCenterMumarenVoucher.voucher_date >= start_date,
        FinanceCenterMumarenVoucher.voucher_date <= end_date,
        func.abs(FinanceCenterMumarenVoucher.total_debit - FinanceCenterMumarenVoucher.total_credit) >= Decimal("0.01"),
    ))
    checks.append({"key": "balanced_vouchers", "title": "凭证借贷平衡", "severity": "error" if unbalanced else "success", "passed": unbalanced == 0, "count": unbalanced, "message": "本期凭证借贷均平衡。" if not unbalanced else f"本期有 {unbalanced} 张凭证借贷不平。"})

    payroll_pending = await _count(db, select(func.count()).select_from(FinanceCenterMumarenPayroll).where(
        FinanceCenterMumarenPayroll.book_id == book_id, FinanceCenterMumarenPayroll.period == period,
        FinanceCenterMumarenPayroll.workflow_status != "paid",
    ))
    checks.append({"key": "payroll_done", "title": "工资已处理", "severity": "warning" if payroll_pending else "success", "passed": payroll_pending == 0, "count": payroll_pending, "message": "本期工资已处理。" if not payroll_pending else f"本期有 {payroll_pending} 条工资尚未发放。"})

    tax_pending = await _count(db, select(func.count()).select_from(FinanceCenterMumarenTaxRecord).where(
        FinanceCenterMumarenTaxRecord.book_id == book_id, FinanceCenterMumarenTaxRecord.period == period,
        FinanceCenterMumarenTaxRecord.status != "paid",
    ))
    checks.append({"key": "tax_paid", "title": "税款已处理", "severity": "warning" if tax_pending else "success", "passed": tax_pending == 0, "count": tax_pending, "message": "本期税款已处理。" if not tax_pending else f"本期有 {tax_pending} 条税款尚未缴清。"})

    invoice_pending = await _count(db, select(func.count()).select_from(FinanceCenterMumarenInvoice).where(
        FinanceCenterMumarenInvoice.book_id == book_id, FinanceCenterMumarenInvoice.invoice_date >= start_date,
        FinanceCenterMumarenInvoice.invoice_date <= end_date, FinanceCenterMumarenInvoice.verification_status != "verified",
    ))
    checks.append({"key": "invoices_verified", "title": "发票已认证", "severity": "warning" if invoice_pending else "success", "passed": invoice_pending == 0, "count": invoice_pending, "message": "本期发票已认证。" if not invoice_pending else f"本期有 {invoice_pending} 张发票尚未认证。"})

    cash_pending = await _count(db, select(func.count()).select_from(FinanceCenterMumarenCashFlow).where(
        FinanceCenterMumarenCashFlow.book_id == book_id, FinanceCenterMumarenCashFlow.flow_date >= start_date,
        FinanceCenterMumarenCashFlow.flow_date <= end_date, FinanceCenterMumarenCashFlow.voucher_id.is_(None),
    ))
    checks.append({"key": "cash_flows_voucher", "title": "出纳流水已生成凭证", "severity": "warning" if cash_pending else "success", "passed": cash_pending == 0, "count": cash_pending, "message": "本期出纳流水均已关联凭证。" if not cash_pending else f"本期有 {cash_pending} 条出纳流水未关联凭证。"})

    receivable_pending = await _count(db, select(func.count()).select_from(FinanceCenterMumarenReceivableOrder).where(
        FinanceCenterMumarenReceivableOrder.book_id == book_id,
        FinanceCenterMumarenReceivableOrder.order_date >= start_date,
        FinanceCenterMumarenReceivableOrder.order_date <= end_date,
        FinanceCenterMumarenReceivableOrder.workflow_status != "posted",
    ))
    payable_pending = await _count(db, select(func.count()).select_from(FinanceCenterMumarenPayableOrder).where(
        FinanceCenterMumarenPayableOrder.book_id == book_id,
        FinanceCenterMumarenPayableOrder.order_date >= start_date,
        FinanceCenterMumarenPayableOrder.order_date <= end_date,
        FinanceCenterMumarenPayableOrder.workflow_status != "posted",
    ))
    ar_ap_pending = receivable_pending + payable_pending
    checks.append({"key": "ar_ap_posted", "title": "应收应付已入账", "severity": "warning" if ar_ap_pending else "success", "passed": ar_ap_pending == 0, "count": ar_ap_pending, "message": "本期应收应付已入账。" if not ar_ap_pending else f"本期有 {receivable_pending} 张应收单、{payable_pending} 张应付单尚未入账。"})

    asset_pending = await _count(db, select(func.count()).select_from(FinanceCenterMumarenFixedAsset).where(
        FinanceCenterMumarenFixedAsset.book_id == book_id,
        FinanceCenterMumarenFixedAsset.purchase_date <= end_date,
        FinanceCenterMumarenFixedAsset.status == "active",
        FinanceCenterMumarenFixedAsset.accumulated_depreciation == 0,
    ))
    checks.append({"key": "asset_depreciation", "title": "固定资产折旧已检查", "severity": "warning" if asset_pending else "success", "passed": asset_pending == 0, "count": asset_pending, "message": "本期固定资产折旧已检查。" if not asset_pending else f"有 {asset_pending} 项固定资产累计折旧为 0，请确认是否需要计提。"})

    profit_loss = (await db.execute(select(func.coalesce(func.sum(
        FinanceCenterMumarenVoucherLine.debit_amount + FinanceCenterMumarenVoucherLine.credit_amount
    ), 0)).select_from(FinanceCenterMumarenVoucherLine).join(
        FinanceCenterMumarenVoucher, FinanceCenterMumarenVoucher.id == FinanceCenterMumarenVoucherLine.voucher_id
    ).join(FinanceCenterMumarenAccount, FinanceCenterMumarenAccount.id == FinanceCenterMumarenVoucherLine.account_id).where(
        FinanceCenterMumarenVoucher.book_id == book_id, FinanceCenterMumarenVoucher.status == "posted",
        FinanceCenterMumarenVoucher.voucher_date >= start_date, FinanceCenterMumarenVoucher.voucher_date <= end_date,
        FinanceCenterMumarenAccount.account_type.in_(["income", "expense"]),
    ))).scalar_one() or 0
    profit_warning = Decimal(str(profit_loss)) >= Decimal("0.01")
    checks.append({"key": "carry_forward", "title": "损益已结转", "severity": "warning" if profit_warning else "success", "passed": not profit_warning, "count": 1 if profit_warning else 0, "message": "本期无损益发生或已处理。" if not profit_warning else "本期存在损益，建议先执行结转损益。"})
    ...
    return ApiResponse.ok(data={"period": period, "can_close": blocking_count == 0, "blocking_count": blocking_count, "warning_count": warning_count, "checks": checks, "message": "可以结账" if blocking_count == 0 else f"有 {blocking_count} 项必须处理后才能结账"})


@router.post("/periods/carry-forward", response_model=ApiResponse)
async def carry_forward_fiscal_period(
    book_id: int = Query(ge=1),
    period: str = Query(pattern=r"^\d{4}-\d{2}$"),
    current_user: SysUser = Depends(require_mumaren_voucher_write),
    db: AsyncSession = Depends(get_db),
):
    """生成当前账簿当前期间的损益结转草稿，幂等且不自动过账。"""
    start_date, end_date = _period_dates(period)
    source_key = f"period-close-carry-forward:{book_id}:{period}"
    existing = (await db.execute(select(FinanceCenterMumarenVoucher).where(
        FinanceCenterMumarenVoucher.book_id == book_id,
        FinanceCenterMumarenVoucher.source_key == source_key,
    ))).scalar_one_or_none()
    if existing is not None:
        return ApiResponse.ok(data={"voucher_id": existing.id, "voucher_no": existing.voucher_no, "status": existing.status}, message="本期损益已经结转")

    rows = list((await db.execute(
        select(
            FinanceCenterMumarenAccount.id,
            FinanceCenterMumarenAccount.account_code,
            FinanceCenterMumarenAccount.account_name,
            FinanceCenterMumarenAccount.account_type,
            func.coalesce(func.sum(FinanceCenterMumarenVoucherLine.debit_amount), 0).label("debit"),
            func.coalesce(func.sum(FinanceCenterMumarenVoucherLine.credit_amount), 0).label("credit"),
        ).select_from(FinanceCenterMumarenVoucherLine)
        ...
        .where(
            ...
            FinanceCenterMumarenAccount.id, FinanceCenterMumarenAccount.account_code,
            FinanceCenterMumarenAccount.account_name, FinanceCenterMumarenAccount.account_type,
        )
    )).all())
    if not rows:
        return ApiResponse.ok(data={"voucher_id": None, "voucher_no": None, "status": "not_needed"}, message="本期没有收入或费用，无需结转")

    profit_account = (await db.execute(select(FinanceCenterMumarenAccount).where(
        FinanceCenterMumarenAccount.book_id == book_id,
        FinanceCenterMumarenAccount.account_code == "4103",
        FinanceCenterMumarenAccount.is_active.is_(True),
    ))).scalar_one_or_none()
    if profit_account is None:
        raise HTTPException(status_code=400, detail="当前账簿缺少 4103 本年利润科目")

    lines = []
    net_profit = Decimal("0")
    for row in rows:
        debit, credit = Decimal(str(row.debit or 0)), Decimal(str(row.credit or 0))
        if row.account_type == "income":
            lines.append({"account_id": row.id, "debit_amount": credit, "credit_amount": Decimal("0"), "summary": f"结转{row.account_name}"})
            net_profit += credit - debit
        else:
            lines.append({"account_id": row.id, "debit_amount": Decimal("0"), "credit_amount": debit, "summary": f"结转{row.account_name}"})
            net_profit -= debit - credit
    if net_profit >= 0:
        lines.append({"account_id": profit_account.id, "debit_amount": Decimal("0"), "credit_amount": net_profit, "summary": "本期净利润转入"})
    else:
        lines.append({"account_id": profit_account.id, "debit_amount": abs(net_profit), "credit_amount": Decimal("0"), "summary": "本期净亏损转入"})

    voucher = await create_mumaren_voucher(
        db, book_id=book_id, voucher_no=f"结转-{period}-{_actor_id(current_user)}",
        voucher_date=end_date, summary=f"{period} 结转损益", voucher_type="结",
        lines=lines, operator_id=_actor_id(current_user),
    )
    voucher.source_key = source_key
    voucher.source_system = "mumaren_period_close"
    await db.flush()
    return ApiResponse.ok(data={"voucher_id": voucher.id, "voucher_no": voucher.voucher_no, "status": voucher.status}, message="损益结转凭证草稿已生成")
...
async def close_fiscal_period(
    ...
    if book_id is not None and period.book_id != book_id:
        raise HTTPException(status_code=400, detail="账簿不一致,禁止跨账簿操作")
